...
export const getStorage = async (client,{type},response) => {
    ...
    const hasDefault = fs.existsSync(defaultPath) && fs.lstatSync(defaultPath).isFile();

    if (!hasMap) {
        console.log(`User ${client.info.num} try get a "${type}" storage, but this is not exist.`)
    };

    if (hasDefault) {
        const usedType = hasMap ? type : DEFAULT_STORAGE;
        const map = JSON.parse(await readFile(path.join(STORAGE_DIR,`/${usedType}/map`)));
        const files = await getStorageFiles(path.join(STORAGE_DIR,`/${usedType}`));

        response({map,files});
    } else {
        client.close(1011,"Unknown error on the server");
        console.log(`Error with default storage.`)
    }
}