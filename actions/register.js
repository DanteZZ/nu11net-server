...
const isUsed = (ip) => {
  ...
  if (
    global.USED_IPS.indexOf(ip) >= 0 &&
    parseInt(ip.split(".")[3]) !== Servers.GATEWAY.gateway_octet
  ) {
    return true;
  }
  return false;
  ...
  if (payload.login.length > 2 && payload.password.length > 4) {
    try {
      ...
      if (!res) {
        ...
        await db.run(
          'INSERT INTO "clients" ("username","password","ip","inf") VALUES (?,?,?,?);',
          [payload.login, encpass, ip, JSON.stringify(genStartInf())]
        );
        logger.info(`new registered user <${payload.login}>`);
      } else {
        status = 401;
      }
    } catch (e) {
